...
async function getProcessContentLinksFunction() {
	...
	const transformContentMatch = buildScript.match(/const transformContent = \(content\) => \{([\s\S]*?)^\};/m);
	const linkMappingsMatch = buildScript.match(/const LINK_MAPPINGS = \{([\s\S]*?)^\};/m);
	const processContentLinksMatch = buildScript.match(/const processContentLinks = \(content, file\) => \{([\s\S]*?)^\};/m);

	if (hasNumericMatch && fixImageMatch && removeHtmlMatch && transformContentMatch && linkMappingsMatch && processContentLinksMatch) {
		return new Function('content', 'file', `
