...
### What Gets Generated and Committed by CI

The CI workflow generates and **commits** the following to the docs repository:

```
src/content/docs/v*/            # Extracted documentation by version (copied from core repo)
src/content/versions/           # Version metadata
astro.config.mjs                # Updated with version information
```

These files are:
- Generated by the CI workflow from the Pepr core repository
- Committed back to the docs repo by the workflow
- Listed in `.gitignore` to hide them from local development (prevents local modifications)

### What Gets Generated but NOT Committed (Temporary)

The following are **generated on every build** and gitignored:

```
work/                           # Temporary clone of core repo
public/_redirects               # Netlify redirect rules (generated during build)
dist/                           # Final built site
```

These are listed in `.gitignore` and never committed.

### What Gets Committed (Source Files)

The following are source files that should be manually committed:

```
build/                          # Build system source code
src/                           # Site source (templates, layouts, components, etc.)
  ├─ layouts/
  ├─ components/
  └─ pages/
...
**Note:** `astro.config.mjs` is both a source file AND gets auto-updated by CI with version information.
