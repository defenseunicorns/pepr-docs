...
:::caution[Unreleased Version]
You are viewing the **main (unreleased)** version of the documentation. This may contain experimental features and changes that are not yet available in stable releases. For stable documentation, please select a specific version (v0.54, v0.53) from the version selector above.
:::

{/* Image styling for Pepr logo */}
...
<CardGrid stagger>
  ...
  <Card title="TypeScript First" icon="open-book">
    Human-readable fluent API for generating [Pepr Capabilities](#capability) with full TypeScript support.
  </Card>

  <Card title="Kubernetes Fluent Client" icon="puzzle">
  ...
  <Card title="NPM Ecosystem" icon="external">
    Entire NPM ecosystem available for advanced operations with out of the box airgap support with [Zarf](https://zarf.dev).
  </Card>

  <Card title="Debugging System" icon="laptop">
    Realtime K8s debugging system for testing/reacting to cluster changes.
  </Card>

  <Card title="Security & RBAC" icon="approve-check">
    Controller network isolation, tamper-resistant module execution, and least-privilege [RBAC](./user-guide/rbac/) generation.
  </Card>
</CardGrid>
...
{/* Architecture diagram temporarily disabled due to SVG processing issues */}
